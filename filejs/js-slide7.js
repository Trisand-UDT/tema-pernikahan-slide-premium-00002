var splide;
        ...
        function toggleMusicSlide(audioLink) {
          if(audioLink){
            // pause yt
            if (playerYT) {
        ...
        $(".awal1").on("click", function () {
            if (!opened) {
                ...
                if ($('#audio-player').length) {
                  ...
                  // if(isYouTube(audioLink)) {
                  //   if (!playerYT) {
                  //     playerYT = new YT.Player('audio-player', {
                  //       height: '0',
                  //       width: '0',
                  //       videoId: videoIdYT,
                  //       playerVars: {
                  ...
                  if (isSoundCloud(audioLink)) {
                    const iframe = Object.assign(document.createElement("iframe"), {
                      width: "1",
                      height: "1",
                      allow: "autoplay",
                      src: "https://w.soundcloud.com/player/?url=" + encodeURIComponent(audioLink),
                      style: "opacity:0;position:absolute;pointer-events:none;"
                    });

                    document.getElementById("audio-player").appendChild(iframe);

                    playerSC = SC.Widget(iframe);

                    playerSC.bind(SC.Widget.Events.PLAY, () => scIsPaused = false);
                    playerSC.bind(SC.Widget.Events.PAUSE, () => scIsPaused = true);
                    playerSC.bind(SC.Widget.Events.READY, () => playerSC.play());
                  }
                ...
                if ($("#playAudioS").length > 0) {
                    $("#playAudioS").get(0).play();
                } //play musik
                runAutoScroll()
                slide('next')
                $('#buka-undangan').hide()
